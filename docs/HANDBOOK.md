...
**netplan** (оба IP на одном интерфейсе; дефолтный шлюз — в подсети расходного IP → egress автоматом с него):
```yaml
network:
  version: 2
  ethernets:
    ens3:
      ...
      routes:
        - to: default
          via: 198.51.100.1      # шлюз в подсети расходного → egress с 198.51.100.20
```
Проверка: `curl ifconfig.me` на ноде → должен вернуть **расходный** IP. Reality слушает `0.0.0.0:443`, так что приём на чистом IP работает без изменений.

**В панели** (профиль этой ноды): выход через `DIRECT` вместо `warp`. В routing поменяй:
```json
{ "type":"field", "network":"tcp,udp", "outboundTag":"warp" }   // было
{ "type":"field", "network":"tcp,udp", "outboundTag":"DIRECT" } // стало
```
`DIRECT` — это `freedom` без метки → трафик идёт через дефолтный маршрут → расходный IP. WARP-outbound можно оставить (метка станет неактивной) или убрать.
...
Проверка с клиента: `2ip` → показывает **расходный** IP (а не Cloudflare и не входной).
